...
df = pd.read_excel(r'M:\citg\se\WR\Moisture Damage\Mahmoud PhD\Publication\TwoScales Paper\Mastic\DNN\Prediction 1\Data_for_NN_ALLINPUTS.xlsx')
...
# Your directory path
dir_path = r'M:\citg\se\WR\Moisture Damage\Mahmoud PhD\Publication\TwoScales Paper\Mastic\DNN\Prediction 1'
...
# Ensure all plots have a white background
...
plt.grid(False)
plt.tight_layout()
plt.savefig(f'{dir_path}\\MSEvsEpochs.png', dpi=300, facecolor='w')
plt.show()
...
for i in range(y_train_original.shape[1]):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'{dir_path}\\Training_PredictionsTarget{i+1}.png', dpi=300, facecolor='w')
    plt.show()
...
for i in range(y_test_original.shape[1]):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'{dir_path}\\Testing_PredictionsTarget{i+1}.png', dpi=300, facecolor='w')
    plt.show()
